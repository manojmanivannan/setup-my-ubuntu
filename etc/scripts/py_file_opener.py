...
try:
    ...
    if str(input('Functionality requires package "Inquirer", proceed to install ? (y/n) ')).lower() in ['yes','y',]:
...
def separate_path_and_file(path_list,match_word):
    ...
    print (OKGREEN+"Common Path:"+ENDC+" "+WARNING+common_dir+ENDC, file=sys.stdout)
    #print("files")
...
def main():
    ...
    questions = [
            inquirer.List('size', message="which file? ",
                choices=file_list,
                )]
