...
s() {
    ...
    # s 'string1\|string2' path -> for OR
        ...
        grep -Hrin "$regex" --include "$filename_ext" "$location";}
...
csv() {
    local filename=${1?param missing - specify full file path}
    column -t -s, -n "$filename" | less -F -S -X -K
}

load_docker_credentials() {
        ~/.scripts/load_docker_credentials.sh
}
...
function gcorecent(){
    local branch=$(git recentb | fzf | cut -d ' ' -f3)
